...
export function ExperimentCard({
  ...
  return (
    <div
      ...
      className={cn(
        "group relative cursor-pointer overflow-hidden rounded-lg border border-border bg-card transition-all hover:border-foreground/20",
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-card-dark/90 to-transparent p-4 opacity-0 transition-opacity group-hover:opacity-100">
        <h3 className="font-semibold text-card-dark-foreground">
          {experiment.title}
        </h3>
        <p className="line-clamp-2 text-card-dark-foreground/70 text-sm">
          {experiment.description}
        </p>
        <div className="mt-2 flex flex-wrap gap-1">
          {experiment.mechanics.map((mechanic) => (
            <span
              className="rounded-full bg-accent/20 px-2 py-0.5 text-accent-foreground text-xs"
              key={mechanic}
            >
              {mechanic}
            </span>
