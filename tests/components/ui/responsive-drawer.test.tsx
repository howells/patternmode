...
describe('ResponsiveDrawer Component', () => {
  const componentDir = join(process.cwd(), 'src', 'components', 'ui', 'responsive-drawer')
  ...
  it('should have main component file', () => {
    const componentFile = join(componentDir, 'responsive-drawer.tsx')
  ...
  it('should be in category: ui', () => {
    expect('ui').toMatch(/^(text|layout|navigation|feedback|overlay|data|media|utility|inputs|forms|charts|ui)$/)
  })
  ...
  it('should be importable directly from component file', async () => {
    try {
      ...
      const componentPath = join(componentDir, 'responsive-drawer.tsx')
