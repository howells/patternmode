...
describe('SelectNative Component', () => {
  const componentDir = join(process.cwd(), 'src', 'components', 'ui', 'select-native')
  ...
  it('should have main component file', () => {
    const componentFile = join(componentDir, 'select-native.tsx')
  ...
  it('should be in category: inputs', () => {
    expect('inputs').toMatch(/^(text|layout|navigation|feedback|overlay|data|media|utility|inputs|forms|charts|ui)$/)
  })
  ...
  it('should be importable directly from component file', async () => {
    try {
      ...
      const componentPath = join(componentDir, 'select-native.tsx')
