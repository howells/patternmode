...
export const tailwindColorVariants = [
  "slate",
  "gray",
  "zinc",
  "neutral",
...
export type TailwindColor = (typeof tailwindColorVariants)[number];
...
export const buttonSpecificVariants = {
  ...
  "outline-dashed": [
    // dashed border (slightly darker than regular outline)
...
export function generateColorClasses(options: ColorClassOptions): {
  ...
  const SHADE_THRESHOLD = 500 as const;
  const LIGHT_BG_SHADE = 50 as const;
  const DARK_BG_SHADE = 900 as const;
  const DEFAULT_DARK_TONE = 400 as const;
  const DEFAULT_BORDER_OPACITY = 30 as const;

  const { color, shade = SHADE_THRESHOLD, bgOpacity, borderOpacity } = options;
  ...
  const bgShade = shade <= SHADE_THRESHOLD ? LIGHT_BG_SHADE : DARK_BG_SHADE;
  const textShade = shade <= SHADE_THRESHOLD ? DARK_BG_SHADE : LIGHT_BG_SHADE;
  const borderShade = shade;

  const bgOpacityStr = bgOpacity ? `/${bgOpacity}` : "";
  const borderOpacityStr = borderOpacity
    ? `/${borderOpacity}`
  ...
  return {
    ...
    dark: {
      bg: `dark:bg-${color}-${DEFAULT_DARK_TONE}/10`,
      text: `dark:text-${color}-${DEFAULT_DARK_TONE}`,
      border: `dark:border-${color}-${DEFAULT_DARK_TONE}${borderOpacityStr}`,
      ring: `dark:ring-${color}-${DEFAULT_DARK_TONE}${borderOpacityStr}`,
    },
  };
}
 ...
 */
export function getColorClasses(variant: SemanticVariant | TailwindColor) {
