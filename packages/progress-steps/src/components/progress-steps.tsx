...
const stateSrText = (state: ProgressStepState) => {
  if (state === "complete") {
    return " – completed";
  }
  if (state === "active") {
...
const connectorColorFor = (state: ProgressStepState) =>
  state === "complete"
    ...
    return (
      ...
      >
        {steps.map((step, idx) => {
          ...
          const connectorColor = connectorColorFor(state);
          const key =
            typeof step.title === "string" ? step.title : `step-${idx}`;
              ...
              <div className={stepItemVariants({ orientation, state })}>
                ...
                <div className="min-w-0 py-0.5">
                  <div className={titleVariants()}>
                    ...
                    <span className="sr-only">{stateSrText(state)}</span>
                  </div>
                  {step.description ? (
