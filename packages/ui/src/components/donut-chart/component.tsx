...
const DonutChart = ({
	...
	className,
	...other
}: DonutChartProps & { ref?: React.RefObject<HTMLDivElement | null> }) => {
	...
	if (!value || !category) {
		...
		return (
			<div
				ref={forwardedRef}
				className={cx(
					"h-40 w-40 flex items-center justify-center text-sm text-zinc-500",
					className,
				)}
	...
	const categories = Array.from(
		new Set(validData.map((item) => item[category])),
	) as string[];
