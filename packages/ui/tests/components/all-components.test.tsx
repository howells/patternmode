...
async function testComponent(componentId: string) {
  ...
  // Skip dynamic import test in UI package - the config files have dependencies on the main app
  // This test is more appropriate for the main app's test suite
  console.log(`  Skipping dynamic import test for ${componentId} (requires main app context)`);
  
  // Just verify the file exists and has basic syntax
  expect(configContent).toContain("export const componentConfig");
...
describe("All UI Components", () => {
  ...
  it("should have all components represented in component registry", () => {
    ...
    console.log(`Found ${componentIds.length} components in /components directory`);
    console.log(`Found ${registryComponents.size} components in registry`);

    // Check that all components in /components directory are in the registry
    ...
    // Check that all components in registry exist in /components directory
    ...
    if (missingFromDirectory.length > 0) {
      console.error(
        "❌ Components in registry but missing from /components directory:",
