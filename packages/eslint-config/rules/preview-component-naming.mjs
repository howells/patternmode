...
const rule = {
	meta: {
		...
		schema: [],
		messages: {
			incorrectPreviewExport:
				"Preview file should export '{{expectedName}}' but found '{{actualName}}'",
			missingPreviewExport:
				"Preview file should export '{{expectedName}}' component",
		},
	},
	...
	create(context) {
		...
		if (
			!(filename.includes("/components/") && filename.endsWith("/preview.tsx"))
		) {
			return {};
		}
		...
		const componentDirIndex = pathParts.findIndex(
		...
		return {
			...
			"Program:exit"() {
				if (!hasCorrectExport) {
					const incorrectExport = foundExports.find((name) =>
