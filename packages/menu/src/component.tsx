...
const MenuContent = ({
  ...
  <BaseMenu.Portal>
    <BaseMenu.Positioner sideOffset={sideOffset} align={align} collisionPadding={collisionPadding}>
      <BaseMenu.Popup
        ref={forwardedRef}
        className={cx(
          // base
...
const MenuItem = ({
  ...
  >
    ...
    {children}
    {hint && <span className={cx("ml-auto pl-2 text-sm text-zinc-400 dark:text-zinc-600")}>{hint}</span>}
    {shortcut && <span className={cx("ml-auto pl-2 text-sm text-zinc-400 dark:text-zinc-600")}>{shortcut}</span>}
  </BaseMenu.Item>
);
...
const MenuCheckboxItem = ({
  ...
  >
    ...
    {children}
    {hint && <span className={cx("ml-auto text-sm font-normal text-zinc-400 dark:text-zinc-600")}>{hint}</span>}
    {shortcut && (
      <span className={cx("ml-auto text-sm font-normal tracking-widest text-zinc-400 dark:border-zinc-800 dark:text-zinc-600")}>{shortcut}</span>
    )}
  </BaseMenu.CheckboxItem>
);
...
const MenuSeparator = ({ ref: forwardedRef, className, ...props }: MenuSeparatorProps) => (
  <BaseMenu.Separator ref={forwardedRef} className={cx("-mx-1 my-1 h-px border-t  dark:border-zinc-800", className)} {...props} />
);
