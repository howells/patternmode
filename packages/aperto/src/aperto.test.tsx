...
describe("Aperto", () => {
  it("opens a single media item from the default thumbnail", async () => {
    ...
    expect(screen.getByRole("dialog", { name: "Morning ridge" })).toBeInTheDocument();
  ...
  it("opens the represented media item from a thumbnail inside a group", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Open Soft afternoon" }));

    expect(screen.getByRole("dialog", { name: "Soft afternoon" })).toBeInTheDocument();
    expect(screen.getByRole("img", { name: "A quiet reading nook" })).toHaveAttribute(
      "src",
      "/second-large.jpg",
    );
  });
  ...
  it("renders centered svg icons for group controls", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Previous media" }).querySelector("svg"),
    ).toBeInTheDocument();
    expect(
  ...
  it("navigates between media items with left and right arrow keys", async () => {
    ...
    expect(screen.getByRole("dialog", { name: "Studio table" })).toBeInTheDocument();
  ...
  it("keeps group content mounted while switching media", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Open Studio table" }));
    const openedContent = document.querySelector('[data-slot="aperto-content"]');

    await user.click(screen.getByRole("button", { name: "Next media" }));

    expect(document.querySelector('[data-slot="aperto-content"]')).toBe(openedContent);
    expect(screen.getByRole("dialog", { name: "Soft afternoon" })).toBeInTheDocument();
  });
  ...
  it("uses the custom image renderer for thumbnails and expanded media", async () => {
    ...
    expect(screen.getByRole("button", { name: "Open Oak and stone" })).toBeInTheDocument();
    expect(container.querySelector('[data-variant="thumbnail"]')).toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: "Open Oak and stone" }));

    expect(screen.getByLabelText("Minimal kitchen shelf")).toHaveAttribute(
      "data-rendered-by",
      "custom-image",
    );
    expect(screen.getByLabelText("Minimal kitchen shelf")).toHaveAttribute(
  ...
  it("renders video media with a thumbnail and expanded controls", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Open Quiet room study" }));

    expect(screen.getByLabelText("A slow interior pan")).toHaveAttribute("src", "/film.mp4");
