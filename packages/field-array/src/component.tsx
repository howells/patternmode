...
export type FieldArrayProps<T extends FieldArrayItem = FieldArrayItem> = {
   ...
   * Components receive a flexible props bag appropriate for the field type.
   */
...
function FieldArray<T extends FieldArrayItem = FieldArrayItem>({
  ...
  > = {
    input: (rawProps: any) => {
      const { value, onChange, ...props } = rawProps as {
        value: string;
    ...
    checkbox: (rawProps: any) => {
      ...
      return (
        <Checkbox
          checked={!!value}
          onCheckedChange={(val: unknown) => onChange(!!val)}
          {...props}
        />
      );
    },
  ...
  const renderField = (
    ...
    if (!Component) {
      // No component found for field type; skip rendering
      return null;
    }
  ...
  return (
    <div className={cx("space-y-4", className)} data-testid="field-array">
      {items.map((item, index) => {
        ...
        if (renderItem) {
          return (
            <div
              className="group relative rounded-lg border bg-white p-4 dark:border-zinc-700 dark:bg-zinc-950"
              key={itemKey}
            >
              {showItemLabels && (
      ...
      {(!maxItems || items.length < maxItems) && (
        <Button leftIcon={Plus} onClick={addItem} variant="outline">
          {addButtonText}
        </Button>
      )}
    </div>
