...
import { SafeDynamicIcon } from "./safe-dynamic-icon";
...
export function useIconSelect(initialValue?: string) {
  ...
  const DynamicIconComponent = React.useMemo(() => {
    ...
    const IconComponent = React.memo(
      ({ className, strokeWidth, ...props }: { className?: string; strokeWidth?: number; [key: string]: unknown }) => (
        <SafeDynamicIcon
          className={className}
          name={value}
...
export function getDynamicIconByName(name: string) {
  const DynamicIconWrapper = React.memo(
    ({ className, strokeWidth, fallback, ...props }: { className?: string; strokeWidth?: number; fallback?: React.ReactNode; [key: string]: unknown }) => {
      return (
        <SafeDynamicIcon className={className} fallback={fallback} name={name} strokeWidth={strokeWidth || DEFAULT_ICON_STROKE_WIDTH} {...props} />
      );
    }
  );
