...
export const radioLabelVariants = tv({
  ...
  variants: { size: { "2xs": "gap-1 text-[11px]", xs: "gap-1 text-xs", sm: "gap-1.5 text-xs", base: "gap-2 text-sm", lg: "gap-2.5 text-base" } },
...
export const radioCardVariants = tv({
  ...
  variants: { size: { xs: `p-2 ${borderRadiusVariants.xs}`, sm: `p-3 ${borderRadiusVariants.sm}`, base: `p-4 ${borderRadiusVariants.base}`, lg: `p-5 ${borderRadiusVariants.lg}` } },
  defaultVariants: { size: "base" },
});
...
export const radioGroupVariants = tv({
  base: ["grid gap-2"],
  variants: {
    orientation: { vertical: "grid-cols-1", horizontal: "grid-flow-col auto-cols-max gap-4" },
