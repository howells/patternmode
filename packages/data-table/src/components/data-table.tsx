...
function createDataTableStore<TData>({
  ...
  return create<StoreState<TData>>((set) => ({
...
export function DataTable<TData>({
  ...
  const storeRef = React.useRef<ReturnType<typeof createDataTableStore<TData>> | null>(null);
  if (!storeRef.current) {
        ...
        <Table className="min-w-full">
          ...
          <TableBody>
            {(() => {
              ...
              return (
                <TableRow>
                  <TableCell className="h-24 text-center text-zinc-500" colSpan={table.getAllColumns().length}>
                    No results
                  </TableCell>
                </TableRow>
              );
