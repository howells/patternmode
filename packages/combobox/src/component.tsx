...
function VirtualizedItemList<T extends ComboboxOption>({
  ...
  const itemHeight = React.useMemo(() => {
    /* biome-ignore lint/nursery/noUnnecessaryConditions: explicit size mapping */
    switch (size) {
      case "2xs":
        return 22;
      case "xs":
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      virtualItems.at(-1)?.index >= items.length - 1 - 5 &&
      hasNextPage &&
      !isFetchingNextPage
    ) {
      fetchNextPage();
...
const Combobox = <T extends ComboboxOption = ComboboxOption>({
  ...
  let queryResult: any;
  try {
    /* biome-ignore lint/correctness/useHookAtTopLevel: optional data fetching hook */
  ...
  const selectedItem = (
    value !== undefined ? controlledSelectedItem : internalSelectedItem
  ) as T | null;
  ...
  const inputProps = getInputProps(
    { placeholder: searchPlaceholder, disabled },
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={cx(
            "rounded-t-md border-b bg-white dark:border-zinc-800 dark:bg-zinc-950",
            size === "xs" && "p-1",
            size === "sm" && "p-1.5",
            size === "base" && "p-2",
            size === "lg" && "p-2.5"
