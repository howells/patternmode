...
const Combobox = <T extends ComboboxOption = ComboboxOption>({
  ...
  React.useEffect(() => {
    ...
    setIsLoading(true);
    setError(null);
    fetchData({ search: debouncedSearch, pageParam: 0, signal: controller.signal })
      .then((res) => {
  ...
  return (
    <div className={cx(comboboxVariants({ size }), className)} data-testid="combobox">
      <BaseCombobox.Root
        // Provide values to Base UI to enable built-in filtering/virtualization.
        items={items.map((it) => getItemValue(it))}
        displayValue={(val: unknown) => {
          const it = valueToItem.get(String(val));
          return it ? String(getItemLabel(it)) : "";
        }}
      ...
      >
        <BaseCombobox.Trigger
          className={cx(
            ...
            "data-[popup-open]:bg-zinc-100 hover:bg-zinc-100"
          )}
          data-testid="combobox-trigger"
        >
          <BaseCombobox.Value>
            {(selected) => {
              const it = selected ? valueToItem.get(String(selected)) : undefined;
              if (!it)
                return (
                  <span className="truncate text-zinc-500 dark:text-zinc-500">{placeholder}</span>
                );
              return (
          ...
          <BaseCombobox.Positioner>
            <BaseCombobox.Popup
              className={cx(
                ...
                "[--input-container-height:3rem] origin-[var(--transform-origin)] max-w-[var(--available-width)] max-h-[min(24rem,var(--available-height))]",
            ...
            >
              <div className="w-80 h-[var(--input-container-height)] p-2">
                <BaseCombobox.Input
                  ...
                  render={({ className, ref, ...renderProps }) => (
                    <Input
                      ...
                      externalRef={ref as unknown as React.RefObject<HTMLInputElement>}
                      size={size}
                      {...renderProps}
                    />
                  )}
              ...
              <div
                className="overflow-y-auto scroll-py-2 py-2 overscroll-contain max-h-[min(calc(24rem-var(--input-container-height)),calc(var(--available-height)-var(--input-container-height)))] empty:p-0"
                ...
                {isMounted ? (
                  <>
                    ...
                    {!(isLoading || error) && items.length > 0 && (
                      <BaseCombobox.List virtualized itemSize={
                        size === "2xs" ? 22 : size === "xs" ? 24 : size === "sm" ? 28 : size === "lg" ? 40 : 34
                      }>
