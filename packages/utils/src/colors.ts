...
function customHash(value: string) {
  ...
  for (let i = 0; i < value.length; i++) {
    // Use Math.imul to avoid bitwise operators while keeping good distribution
    hash = Math.imul(HASH_MULTIPLIER, hash) + value.charCodeAt(i);
    if (hash < 0) {
      hash = -hash;
    }
  }
 ...
 */
function getColor(value: string, arrayLength: number) {
