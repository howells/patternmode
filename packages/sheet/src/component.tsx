 ...
 */
const SheetTrigger = ({ ref, className, ...props }: SheetTriggerProps & { ref?: React.RefObject<React.ElementRef<typeof Dialog.Trigger> | null> }) => {
 ...
 */
const SheetClose = ({ ref, className, ...props }: SheetCloseProps & { ref?: React.RefObject<React.ElementRef<typeof Dialog.Close> | null> }) => {
 ...
 */
const SheetOverlay = ({ ref: forwardedRef, className, ...props }: SheetOverlayProps & { ref?: React.RefObject<React.ElementRef<typeof Dialog.Backdrop> | null> }) => {
  return (
    <Dialog.Backdrop
      ref={forwardedRef}
      className={cx(
 ...
 */
const SheetContent = ({ ref: forwardedRef, className, ...props }: SheetContentProps & { ref?: React.RefObject<React.ElementRef<typeof Dialog.Popup> | null> }) => {
  return (
    <SheetPortal>
      <SheetOverlay />
      <Dialog.Popup
 ...
 */
const SheetHeader = ({ ref, children, className, ...props }: SheetHeaderProps & { ref?: React.RefObject<HTMLDivElement | null> }) => {
  return (
    <div ref={ref} className="flex items-start justify-between gap-x-4 border-b  pb-4 dark:border-zinc-900" {...props}>
      <div className={cx("mt-1 flex flex-col gap-y-1", className)}>{children}</div>
      <Dialog.Close
        render={<InlineSheetDismissButton size="lg" aria-label="Close sheet" className="mt-1 shrink-0" />}
      />
    </div>
  );
};
 ...
 */
const SheetTitle = ({ ref: forwardedRef, className, ...props }: SheetTitleProps & { ref?: React.RefObject<React.ElementRef<typeof Dialog.Title> | null> }) => (
  <Dialog.Title
    ref={forwardedRef}
    className={cx(
      // base
 ...
 */
const SheetBody = ({ ref, className, ...props }: SheetBodyProps & { ref?: React.RefObject<HTMLDivElement | null> }) => {
 ...
 */
type SheetDescriptionProps = React.ComponentPropsWithoutRef<typeof Dialog.Description>;

/**
 * Sheet description component for explanatory text.
 */
const SheetDescription = ({ ref: forwardedRef, className, ...props }: SheetDescriptionProps & { ref?: React.RefObject<React.ElementRef<typeof Dialog.Description> | null> }) => {
  return (
    <Dialog.Description ref={forwardedRef} className={cx("text-zinc-500 dark:text-zinc-500", className)} {...props} />
  );
};
