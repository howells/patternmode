...
export const TextList = ({ as = "ul", variant, align, unstyled, className, children, ...props }: TextListProps) => {
  const Component: React.ElementType = as || "ul";
  return (
    <TextListContext.Provider value={{}}>
      <Component data-testid="text-list" className={cx(!unstyled && listVariants({ variant, align }), className)} {...props}>
        {children}
      </Component>
...
export function TextListItem({ variant, align, unstyled, className, children, ...props }: TextListItemProps) {
  useContext(TextListContext);
  return (
    <li className={cx(!unstyled && listItemVariants({ variant, align }), className)} {...props}>
      {children}
    </li>
  );
}
