...
describe("ScrollFrame", () => {
	it("renders a named Radix-backed scroll region with fades and plumbing", () => {
		...
		expect(region).toHaveClass("howells-scroll-frame");
		expect(region).toHaveAttribute("data-axes", "vertical");
		expect(region).toHaveAttribute("data-axis-vertical", "true");
		expect(region).toHaveAttribute("data-slot", "scroll-frame");

		const viewport = screen.getByTestId("scroll-frame-viewport");
		expect(viewport).toHaveAttribute("data-slot", "scroll-frame-viewport");
		expect(
			screen.getByTestId("scroll-frame-fade-vertical-start"),
		).toBeInTheDocument();
		expect(
			screen.getByTestId("scroll-frame-fade-vertical-end"),
		).toBeInTheDocument();
	});
	...
	it("supports both axes and keeps hidden scrollbar plumbing mounted", () => {
		...
		const root = screen.getByTestId("scroll-frame-viewport").parentElement;
		expect(root).toHaveAttribute("data-axes", "both");
		expect(root).toHaveAttribute("data-scrollbar-visibility", "hidden");
		expect(
			root?.querySelectorAll('[data-slot="scroll-frame-scrollbar"]'),
		).toHaveLength(2);
	});
