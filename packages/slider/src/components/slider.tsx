...
const Slider = ({
  ...
  const {
    ...
    accent: (props as { accent?: boolean }).accent,
  });
  ...
  const currentArray =
    isControlled && value != null ? toArray(value) : internal;
  ...
  const handlePointerDown: React.PointerEventHandler<HTMLDivElement> = (e) => {
    ...
    const min = (props as { min?: number }).min ?? DEFAULT_MIN;
    const max = (props as { max?: number }).max ?? DEFAULT_MAX;
    const step = (props as { step?: number }).step ?? DEFAULT_STEP;
    let ratio = 0;
    if (horizontal) {
