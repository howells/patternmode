...
function getPaddingClasses(avatar: TagProps["avatar"], dismissible: boolean): string {
  if (avatar) {
...
const Tag = ({
  ...
  const defaultProps: useRender.ElementProps<"span"> & { "data-testid": string } = {
    ...
    children: (
      <>
        {avatar && <Avatar src={avatar.src} alt={avatar.alt} initials={avatar.initials} size="xs" className="size-6" />}
        {effectiveIcon && !avatar && <Icon icon={effectiveIcon} size="xs" strokeWidth={iconStrokeWidth} />}
        {label && (
          <>
        ...
        <span className="text-xs font-medium text-zinc-900 dark:text-zinc-50">
          ...
          {count !== undefined && count !== "" && (
            <span className={cx("ml-1.5 text-xs font-normal text-zinc-500 dark:text-zinc-400", countClassName)}>{count}</span>
          )}
        </span>
        {dismissible && <DismissButton onClick={onDismiss} size="xs" className={cx(count !== undefined && count !== "" ? "-ml-1.5" : "-ml-1")} aria-label={dismissAriaLabel} />}
      </>
    ) as React.ReactNode,
  };

  const element = useRender({ render, ref: forwardedRef, props: mergeProps<"span">(defaultProps, props) });
  return element;
};
