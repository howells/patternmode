...
for (const dir of packageDirs) {
  ...
  if (src.includes("@base-ui-components/react"))
    ensureDep("@base-ui-components/react", "^1.0.0-beta.2");
  if (src.includes("'lucide-react'") || src.includes('"lucide-react"'))
    ensureDep("lucide-react", ">=0.525.0 <1");
  if (
    src.includes("'tailwind-variants'") ||
    src.includes('"tailwind-variants"')
  )
    ensureDep("tailwind-variants", "^3.1.0");
  if (src.includes("@patternmode/utils"))
    ensureDep("@patternmode/utils", "workspace:*");
  if (src.includes("@patternmode/config"))
    ensureDep("@patternmode/config", "workspace:*");
...
console.log(
  "peerDependencies migrated where applicable and tsconfigs normalized."
);
